...
alias global niri-terminal-horizontal wayland-terminal-window # can't use niri-terminal-window here because it's an alias

define-command niri-focus-pid -hidden %{
    evaluate-commands %sh{
        ...
        while true; do
            ...
            if [ -z $pid ] || [ $pid -le 1 ]; then
                echo "fail Can't find PID of window to focus"
                break
            fi

            window_id="$(printf '%s\n' $pid "$windows" | jq -s -r '. as [$pid, $windows] | $windows | map(select(.pid == $pid)) | .[0].id // ""')"
...
define-command niri-focus -params ..1 -docstring '
niri-focus [<kakoune_client>]: focus a given client''s window
If no client is passed, then the current client is used' \
%{
    evaluate-commands %sh{
        if [ $# -eq 1 ]; then
            printf "evaluate-commands -client '%s' niri-focus-pid" "$1"
        else
            echo niri-focus-pid
        fi
    }
